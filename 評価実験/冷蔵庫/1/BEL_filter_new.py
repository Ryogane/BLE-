...
def plot_negaposi(t, rssi, stable_time, interval, negaposi, save_name):
    ...
    plt.title('冷蔵庫の状態推定結果', fontsize=18)  # グラフタイトル
    plt.xlabel('time(sec)', fontsize=18)
    plt.ylabel('signal', fontsize=18)
    #plt.xticks(np.arange(0, 120, 10))  # グラフのメモリ間隔の設定
    #ax.xaxis.grid(linestyle='--', lw=1, color='black')
...
def create_lowpath_data_graph(relative_time, rssi, save_name):
    ...
    # 正解ラベルをつける
    true_label = [30,38, 92,98, 180,187, 301,307, 391,397, 431,438, 511,517, 581,587]
    for i in range(0,16,2):
        plt.fill_between([true_label[i], true_label[i+1]], [0,0], [1,1], facecolor='r', alpha=0.6)

    true_label2 = [0,30, 38,92, 98,180, 187,301, 307,391, 397,431, 438,511, 517,581, 587,600]
...
def main():
    ...
    # csvファイルを読み込む
    time, rssi_raw = read_csv_value('./冷蔵庫アダプター1.csv')
    ...
    sz_interval = 2          # 安定センシング区間を判断する時間間隔
    sz_under = 0.23            # 安定センシング区間だと判断する閾値の下限
    sz_upper = 0.23            # 安定センシング区間だと判断する閾値の上限
