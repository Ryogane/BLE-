...
def get_file_path():
    ...
    # ファイルの一覧を取得する
    fltr_list = [filename for filename in os.listdir(dir) if not filename.startswith('.') and filename !='実験の方法.txt' and filename !='BEL_filter_multi.py' and filename!='result']

    num = []
    for file in fltr_list:
        num.append(re.sub("\\D", "", file))

    for file in num:
        csv_path.append('./'+file+'/冷蔵庫の開け締め.csv')
    print(csv_path)
    return csv_path
...
def up_down_judge(stable_rssi):
    ...
    up_down = []
    rssi_median = statistics.median(rssi_ave)           # 各安定センシング区間のrssi値の平均値の中央値を算出
...
def plot_negaposi(t, rssi, stable_time, interval, negaposi, save_name):
    ...
    plt.title('安定センシング区間(ネガポジ)', fontsize=18)  # グラフタイトル
    plt.xlabel('time(sec)', fontsize=18)
    plt.ylabel('signal', fontsize=18)
    plt.xticks(np.arange(0, 120, 10))  # グラフのメモリ間隔の設定
    ax.xaxis.grid(linestyle='--', lw=1, color='black')
...
def create_lowpath_data_graph(relative_time, rssi, save_name):
    ...
    plt.title("ローパスフィルタをかけたrssi値", fontsize=18)      # グラフタイトル
    plt.xlabel("time(sec)", fontsize=18)                      # x軸のラベル
...
def main():
    ...
    # フォルダ内の全てのcsvファイルへのパスを作成する
    all_path = get_file_path()

    # 結果保存用のフォルダを作成する
    dirs = ['./result', './result/move_ave', './result/raw', './result/安定センシング区間', './result/安定センシング区間(ネガポジ)']
    make_dir(dirs)
    ...
    for path in all_path:
        ...
        # csvファイルを読み込む
        time, rssi_raw = read_csv_value(path)
        ...
        sz_interval = 1                 # 安定センシング区間を判断する時間間隔
        sz_under = 0.2                  # 安定センシング区間だと判断する閾値の下限
        sz_upper = 0.2                  # 安定センシング区間だと判断する閾値の上限

        # 保存グラフ名
        raw_graph_name = './result/raw/'+count+'raw_rssi.png'
        move_ave_graph_name = './result/move_ave/'+count+'move_ave_rssi.png'
        StableZone_graph_name = './result/安定センシング区間/'+count+'stable_zone.png'
        negaposi_graph_name = './result/安定センシング区間(ネガポジ)/'+count+'stable_zone_negaposi.png'
        ...
        #write_csv('./result/'+str(count)+'move_ave_rssi.csv', time, rssi)
